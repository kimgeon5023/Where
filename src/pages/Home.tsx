...
export default function Home() {
  ...
  return (
    <main className="app-shell home-shell">
      <header className="topbar"><div className="brand"><span className="brand-mark">W</span><span>어디갈까<span className="brand-dot">.</span></span></div><div className="topbar-actions"><div className="topbar-note"><span className="live-dot" /> 서울 곳곳의 오늘</div><Link to="/saved" className="saved-count">♡ 저장한 코스 {savedCount}</Link><AuthActions /></div></header>
      ...
      <section className="form-card">
        ...
        <div className="form-section"><label className="field-label"><Icon name="sun" size={15} /> 날씨는 어떤가요?</label><div className="segmented"><button type="button" className={request.weather === 'sunny' ? 'selected' : ''} onClick={() => update('weather', 'sunny')}><Icon name="sun" size={16} /> 맑음</button><button type="button" className={request.weather === 'cloudy' ? 'selected' : ''} onClick={() => update('weather', 'cloudy')}><Icon name="cloud" size={16} /> 흐림</button><button type="button" className={request.weather === 'rain' ? 'selected' : ''} onClick={() => update('weather', 'rain')}><Icon name="rain" size={16} /> 비</button></div><p className="field-hint">날씨에 따라 실내·야외 장소가 달라져요.</p></div>
