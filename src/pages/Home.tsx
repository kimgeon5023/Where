  ...
  return (
    <main className="app-shell home-shell">
      ...
      <section className="form-card">
        ...
        <div className="form-section"><label className="field-label"><Icon name="users" size={15} /> 누구와 함께하나요?</label><div className="companion-grid">{companions.map((item) => <button type="button" key={item.value} onClick={() => update('companion', item.value)} className={'companion-card' + (request.companion === item.value ? ' selected' : '')}><span className="companion-icon"><Icon name={item.icon} size={23} /></span><strong>{item.label}</strong><small>{item.caption}</small></button>)}</div>{request.companion === 'couple' && <p className="field-hint party-notice">바람 피는 행동은 나빠요</p>}</div>
