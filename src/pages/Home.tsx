  ...
  return (
    <main className="app-shell home-shell">
      ...
      <section className="form-card">
        ...
        <div className="form-section"><label className="field-label"><Icon name="users" size={15} /> 누구와 함께하나요?</label><div className="companion-grid">{companions.map((item) => <button type="button" key={item.value} onClick={() => { update('companion', item.value); if (item.value === 'couple') update('headcount', 2) }} className={'companion-card' + (request.companion === item.value ? ' selected' : '')}><span className="companion-icon"><Icon name={item.icon} size={23} /></span><strong>{item.label}</strong><small>{item.caption}</small></button>)}</div></div>
        <div className="split-fields"><div className="form-section"><label className="field-label"><Icon name="users" size={15} /> 몇 명인가요?</label>{request.companion === 'couple' && <p className="field-hint party-notice">바람 피는 행동은 나빠요</p>}<div className="stepper"><button type="button" onClick={() => update('headcount', request.companion === 'couple' ? 2 : Math.max(1, request.headcount - 1))}><Icon name="minus" size={15} /></button><strong>{request.headcount}<small>명</small></strong><button type="button" onClick={() => update('headcount', request.companion === 'couple' ? 2 : request.headcount + 1)}><Icon name="plus" size={15} /></button></div></div><div className="form-section"><label className="field-label"><Icon name="card" size={15} /> 1인 예산</label><div className="input-wrap budget-input"><input type="number" min="0" step="10000" value={request.budgetPerPerson} onChange={(event) => update('budgetPerPerson', Number(event.target.value))} /><span>원</span></div></div></div>
