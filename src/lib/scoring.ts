...
function groupSizeFit(place: Place, headcount: number) {
  ...
  if (headcount <= 4) return profile?.space === 'large' ? 12 : profile?.space === 'medium' ? 10 : 7
  if (profile?.space === 'large') return profile.reservationCheckRecommended ? 14 : 13
  if (profile?.space === 'medium') return 8
  return 3
}
...
export function recommend(places: Place[], req: TripRequest, excludedIds: string[] = [], seed = 0): ScoredPlace[] {
  return places.filter((place) => !excludedIds.includes(place.id) && !place.tags.some((tag) => req.dislikes.includes(tag))).map((place) => {
    ...
    const detail = [{ label: '취향 일치', max: 28, value: taste }, { label: '동행 적합', max: 18, value: group }, { label: '인원 적합', max: 14, value: groupSize }, { label: '여행 스타일', max: 14, value: style }, { label: '예산 적합', max: 18, value: budget }, { label: '날씨 적합', max: 18, value: weather }, { label: '이동 편의', max: 18, value: travel }]
    const reasons = [...(liked ? ['선택한 취향과 잘 맞아요.'] : []), ...(companionStyle[req.companion].includes(place.category) ? ['동행 유형에 어울리는 장소예요.'] : []), ...(req.headcount >= 5 && groupSize >= 12 ? [`${req.headcount}명이 함께 가기 좋은 넓은 공간 유형이에요.`] : []), ...(req.headcount >= 5 && place.groupProfile?.reservationCheckRecommended ? ['단체 이용 전 예약 가능 여부를 확인해 주세요.'] : []), ...(place.distanceKm !== undefined ? [`출발 기준 ${place.distanceKm.toFixed(1)}km 거리예요.`] : [])]
